...
class HistoryTest(TestCase):
    ...
    def test_history_detail(self) -> None:
    ...
    def test_order_create_delivery(self) -> None:
        url = reverse('order_create_delivery')
        data = {'delivery': 'A', 'city': 'test', 'address': 'test'}
        response = self.client.post(url, data=data, follow=True)
        self.assertEqual(response.status_code, 200)

    def test_order_type_payment(self) -> None:
        url = reverse('order_type_payment')
        data = {'payment': 'C'}
        response = self.client.post(url, data=data, follow=True)
        self.assertEqual(response.status_code, 200)

    def test_order_comment(self) -> None:
        url = reverse('order_create_comment')
        data = {'comment': 'test'}
        response = self.client.post(url, data=data, follow=True)
        self.assertEqual(response.status_code, 200)

    def test_order_create_payment(self) -> None:
